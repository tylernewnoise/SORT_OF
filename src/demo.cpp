...
void visualize(const std::vector<cv::Mat>&,
        const std::size_t&,
...
int main(int argc, char* argv[])
{
  ...
  if (!display) std::cout << "Running without visualization.\n" << "Use "
                          << argv[0] << " [-d] to visualize"
  ...
  std::cout << "Overall process time: " << timespan_local.count() << "\n"
            ...
            << std::endl;
  if (display) std::cout << "Note: get real runtime results without the option "
                            "[-d]." << std::endl;
  return 0;
}
 ...
 *                     format to file.
 */
void demoSort(const std::string& seqName, const bool& display, const bool& write_output)
{
  ...
  if (write_output) {
    ...
    if (out_file.is_open()) std::cout << "Tracking result in MOT format is "
                                         "written to " << out_file_path
                                         << std::endl;
    else std::cerr << "Error, can not write output file to " << out_file_path
                   << std::endl;
  }
  ...
  sort_of::SORTOF of_sort_tracker = SORTOF(iou_threshold, max_age, max_corners, n_init);
 ...
 *                    numbers.
 */
std::map<std::size_t, std::vector<BBox>>
readDetections(const std::string& seqName)
{
 ...
 */
void visualize(const std::vector<cv::Mat>& images,
        ...
        const std::vector<BBox >& detections)
{
  ...
  for (const auto& t : tracks) {
    ...
    cv::rectangle(img, top_left, cv::Point(top_left.x + 10 + text_size.width ,top_left.y + 10 + text_size.height),
